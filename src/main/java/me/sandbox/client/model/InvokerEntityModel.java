...
public class InvokerEntityModel<T extends IllagerEntity>
    ...
    public static TexturedModelData getTexturedModelData() {
        ...
        modelPartData.addChild(EntityModelPartNames.BODY, ModelPartBuilder.create().uv(16, 20).cuboid(-4.0f, 0.0f, -3.0f, 8.0f, 12.0f, 6.0f).uv(0, 38).cuboid(-4.0f, 0.0f, -3.0f, 8.0f, 23.0f, 6.0f, new Dilation(0.5f)), ModelTransform.pivot(0.0f, 0.0f, 0.0f));
    ...
    public void setAngles(T illagerEntity, float f, float g, float h, float i, float j) {
        ...
        this.root.pitch = MathHelper.sin((float)(illagerEntity).age*0.1f) * ((float)Math.PI / 180);
        this.root.roll = MathHelper.sin((float)(illagerEntity).age*0.1f)* ((float)Math.PI / 180);
        if (illagerEntity.getVelocity().x != 0 || illagerEntity.getVelocity().z !=0) {
            this.body.pitch = 0.15f*g;
            this.rightLeg.pitch = 0.15f*g;
            this.leftLeg.pitch= 0.15f*g;
        }
        if (this.riding) {
