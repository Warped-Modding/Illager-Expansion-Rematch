...
public class CapedIllagerEntityModel<T extends IllagerEntity>
    ...
    public void setAngles(T illagerEntity, float f, float g, float h, float i, float j) {
        ...
        if (state == IllagerEntity.State.ATTACKING) {
            ...
            this.cape.pitch = (MathHelper.cos(h * 0.5222f) * 0.2f) + 0.15f;
            ...
            this.leftArm.roll = -2.3561945f;
            this.leftArm.yaw = 0.0f;
        } else if (state == IllagerEntity.State.CROSSED) {
            this.cape.pitch = (MathHelper.cos(f * 0.3111f + (float)Math.PI) * 0.3f * g * 0.5f)  + 0.15f;
        }
