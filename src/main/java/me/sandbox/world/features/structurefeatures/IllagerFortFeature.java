...
public class IllagerFortFeature extends StructureFeature<StructurePoolFeatureConfig> {
    ...
    public IllagerFortFeature(Codec<StructurePoolFeatureConfig> codec) {
        super(codec, IllagerFortFeature::createPiecesGenerator);
    }
    ...
    public static Optional<StructurePiecesGenerator<StructurePoolFeatureConfig>> createPiecesGenerator(StructureGeneratorFactory.Context<StructurePoolFeatureConfig> context) {
        if (!IllagerFortFeature.canGenerate(context)) {
        ...
        StructurePoolFeatureConfig newConfig = new StructurePoolFeatureConfig(() -> context.registryManager().get(Registry.STRUCTURE_POOL_KEY)
                .get(new Identifier(Sandbox.MOD_ID, "illager_fort/illager_fort")), 1);
