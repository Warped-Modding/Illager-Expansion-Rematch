...
public class SorcererHutFeature extends StructureFeature<StructurePoolFeatureConfig> {
    ...
    public SorcererHutFeature(Codec<StructurePoolFeatureConfig> codec) {
        super(codec, SorcererHutFeature::createPiecesGenerator);
    }
    ...
    public static Optional<StructurePiecesGenerator<StructurePoolFeatureConfig>> createPiecesGenerator(StructureGeneratorFactory.Context<StructurePoolFeatureConfig> context) {
        if (!SorcererHutFeature.canGenerate(context)) {
        ...
        StructurePoolFeatureConfig newConfig = new StructurePoolFeatureConfig(() -> context.registryManager().get(Registry.STRUCTURE_POOL_KEY)
                        .get(new Identifier(Sandbox.MOD_ID, "sorcerer_hut/sorcerer_hut")), 1);
